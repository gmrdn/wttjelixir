defmodule Wttjelixir do
  ...
  def output_nb_job_by_type(joblist) do
    map = group_jobs_by_type(joblist)
  ...
  def output_nb_job_by_type_and_category(joblist) do
    Enum.each(joblist, fn ({key, value}) ->   
      IO.puts("#{Enum.join(Tuple.to_list(key), " ")}: #{length(value)}") 
    end)
  end
